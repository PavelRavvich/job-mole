...
в `.env`). Playwright отдаёт сырой HTML/текст найденных карточек вакансий;
структурирование этого текста в `VacancyRaw` (заголовок, компания, описание,
локация, external_id) делает не хардкод CSS-селекторов, а вызов LLM с ролью
`web` (см. ниже) — так источник переживает мелкие изменения вёрстки сайта.
...
- **`web`** — извлечение структуры из сырых веб-страниц: сейчас — парсинг
  вакансий в `sources/linkedin.py` перед записью в стор; в Фазе 2 та же роль
...
- **Unit**: `resume/parse.py` с фикстур-текстом и замоканным LLM-клиентом
  ...
  `sources/linkedin.py` — с фикстур-HTML и замоканным LLM-клиентом (роль
  `web`), отдельно от самого Playwright-скрейпинга; `agent/nodes.py::match` с
...
- **Ручные/интеграционные**: сам Playwright-скрейпинг в `sources/linkedin.py`
  требует реальной залогиненной сессии — не гоняется в CI, проверяется
